...
#define SERIAL_BAUD_RATE 9600 //9600          // 9600
...
#define AZIMUTH_SMOOTHING_FACTOR 90     // value = 0 to 99.9
#define ELEVATION_SMOOTHING_FACTOR 90    // value = 0 to 99.9
...
int raw_azimuth_pot = 0;
int azimuth_pot = 0;
int raw_azimuth_lsm = 0;
int azimuth_lsm = 0;
...
void update_display()
{
  ...
  if ((millis()-last_lcd_update) > LCD_UPDATE_TIME) {
    ...
    if ((last_azimuth != azimuth) || (last_elevation != elevation)){
      ...
      ////Rich T. added this
      //dtostrf(roll,1,LCD_DECIMAL_PLACES, rollstring);
      //direction_string.concat("R");
      ///direction_string.concat(rollstring);
...
void read_azimuth(){
  ...
  ///////////added by Rich T.
  unsigned int previous_raw_azimuth_pot = raw_azimuth_pot;
  unsigned int previous_raw_azimuth_lsm = raw_azimuth_lsm;
  ...
  /////////////////////changed all raw_azimuth in the section to raw_azimuth_pot and all previous_raw_azimuth to previous_raw_azimuth_pot and all azimuth to azimuth_pot
    analog_az = analogRead(rotator_analog_az);
    raw_azimuth_pot = (map(analog_az, configuration.analog_az_full_ccw, configuration.analog_az_full_cw, (configuration.azimuth_starting_point*HEADING_MULTIPLIER), ((configuration.azimuth_starting_point + configuration.azimuth_rotation_capability)*HEADING_MULTIPLIER)));
    raw_azimuth_pot = raw_azimuth_pot - (180 * HEADING_MULTIPLIER);
    #ifdef FEATURE_AZIMUTH_CORRECTION
    raw_azimuth_pot = (correct_azimuth(raw_azimuth_pot/HEADING_MULTIPLIER)*HEADING_MULTIPLIER);
    #endif //FEATURE_AZIMUTH_CORRECTION
    if (AZIMUTH_SMOOTHING_FACTOR > 0) {
      raw_azimuth_pot = (raw_azimuth_pot*(1-(AZIMUTH_SMOOTHING_FACTOR/100))) + (previous_raw_azimuth_pot*(AZIMUTH_SMOOTHING_FACTOR/100));
    }  
    if (raw_azimuth_pot >= (360 * HEADING_MULTIPLIER)) {
      azimuth_pot = raw_azimuth_pot - (360 * HEADING_MULTIPLIER);
        if (azimuth_pot >= (360 * HEADING_MULTIPLIER)) {
          azimuth_pot = azimuth_pot - (360 * HEADING_MULTIPLIER);
        }
    } else {
      if (raw_azimuth_pot < 0) {
        azimuth_pot = raw_azimuth_pot + (360 * HEADING_MULTIPLIER);
      } else {
        azimuth_pot = raw_azimuth_pot;
      }
    }
    ...
    //// added by Rich T. for azimuth smoothing from LSM
    float heading = lsm.heading();
     if(heading < 0) heading +=360;
    if(heading > 360) heading -= 360;
    raw_azimuth_lsm = (heading ) * HEADING_MULTIPLIER; //radians to degree
    if (AZIMUTH_SMOOTHING_FACTOR > 0) {
      raw_azimuth_lsm = (raw_azimuth_lsm*(1-(AZIMUTH_SMOOTHING_FACTOR/100))) + (previous_raw_azimuth_lsm*(AZIMUTH_SMOOTHING_FACTOR/100));
    }    
    ///////////////////////////////////////////////
    
    //// original code/////////
    //float heading = atan2(lsm.m.y,lsm.m.x);
    //  heading += declinationAngle;
    // Correct for when signs are reversed.
    //if(heading < 0) heading += 2*PI;
    //if(heading > 2*PI) heading -= 2*PI;
    ...
    #ifdef FEATURE_AZIMUTH_CORRECTION
    raw_azimuth_lsm = (correct_azimuth(raw_azimuth_lsm/HEADING_MULTIPLIER)*HEADING_MULTIPLIER);
    #endif //FEATURE_AZIMUTH_CORRECTION
    
    ///also aded by Rich t.
    ///////////////////////////////////////////////////////////////////////////////////
    //raw_azimuth = lsm.heading();
    //azimuth = lsm.heading();
    //azimuth = raw_azimuth;
    /////////////////////////////////////////////////////////////////////////
    ///////////
    azimuth_lsm = raw_azimuth_lsm;
...
void read_elevation()
{
  ...
  unsigned int previous_elevation_pot = elevation_pot;
  unsigned int previous_elevation_lsm = elevation_lsm;
  ...
  if (1){
    ...
    /// replaced elevation with elevation_pot and previous_elevation with previous_elevation_pot
    analog_el = analogRead(rotator_analog_el);
    elevation_pot = (map(analog_el, configuration.analog_el_0_degrees, configuration.analog_el_max_elevation, 0, (ELEVATION_MAXIMUM_DEGREES* HEADING_MULTIPLIER))) ;
    
     //added to make 0 -> 180 and 180 -> 0 so we dont have to remount antenna.
    elevation_pot = abs(elevation_pot - 180);
 
    
    //
    
    #ifdef FEATURE_ELEVATION_CORRECTION
    elevation_pot = (correct_elevation(elevation_pot/HEADING_MULTIPLIER)*HEADING_MULTIPLIER);
    #endif //FEATURE_ELEVATION_CORRECTION
    if (ELEVATION_SMOOTHING_FACTOR > 0) {
      elevation_pot = (elevation_pot*(1-(ELEVATION_SMOOTHING_FACTOR/100))) + (previous_elevation_pot*(ELEVATION_SMOOTHING_FACTOR/100));
    }
    if (elevation_pot < 0) {
      elevation_pot = 0;
    }
    ...
    if (debug_mode) {
      ...
      Serial.println(event.acceleration.z);a
    }
    #endif //DEBUG_ACCEL
    ...
    ////replace elevation with elevation_lsm and previous_elevation with previous_elevation_lsm
    lsm.read();
    #ifdef DEBUG_ACCEL
    if (debug_mode) {
      Serial.print(F("read_elevation: lsm.a.y: "));
      Serial.print(lsm.a.y );
      Serial.print(F(" lsm.a.z: "));
      Serial.println(lsm.a.z );
    }
    #endif //DEBUG_ACCEL
    
    //orig
   /// elevation = (atan2(lsm.a.y,lsm.a.z)* 180 * HEADING_MULTIPLIER)/M_PI;
    
   ////
    
    /////////////added///////
    
    elevation_lsm = (atan2(lsm.a.y,lsm.a.z)* 180 * -1 *HEADING_MULTIPLIER)/M_PI;
    if (ELEVATION_SMOOTHING_FACTOR > 0) {
        elevation_lsm = (elevation_lsm*(1-(ELEVATION_SMOOTHING_FACTOR/100))) + (previous_elevation_lsm*(ELEVATION_SMOOTHING_FACTOR/100));
      }    
    ...
    ///roll
    roll = (atan2(-1*lsm.a.x,lsm.a.z)* 180 * HEADING_MULTIPLIER)/M_PI;
    roll = roll ;
    ////
    
    #ifdef FEATURE_ELEVATION_CORRECTION
    elevation_lsm = (correct_elevation(elevation_lsm/HEADING_MULTIPLIER)*HEADING_MULTIPLIER);
    #endif //FEATURE_ELEVATION_CORRECTION           
...
void initialize_peripherals(){
 ...
 LSM303::deviceType device = LSM303::device_DLM;
 LSM303::sa0State sa0 = LSM303::sa0_low;
